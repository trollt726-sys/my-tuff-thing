...
const isUnusedRemovableTable = (a) => {
    ...
    const walkTable = (node) => {
        if (hasSideEffects || !node || typeof node !== "object") return;
        if (node.type === "CallExpression" || node.type === "StringCallExpression" || node.type === "TableCallExpression") {
            hasSideEffects = true;
            return;
        }
        if (Array.isArray(node)) {
            node.forEach(walkTable);
            return;
        }
        for (const key in node) walkTable(node[key]);
    };
    walkTable(fields);
...
const inline = (
    ...
    const inlineStat = (variable, oldUses, valueStat, y) => {
        ...
        valueStat.init.splice(idx, 1);
        valueStat.variables.splice(idx, 1);
    ...
    const removeFromValue = (value, idx) => {
        ...
        value.init.splice(idx, 1);
        if (!value.init.length) {
            // it's already empty, we can clear.
            return Clear(value);
        }
        value.variables.splice(idx, 1);
    };
    ...
    const inlineThing = (value, list, variable, reassigned, iReallyDc, skip, isFinally) => {
        if (!value || !value.init)
        ...
        if (!skip) {
            ...
            if (init.type == "Identifier" && uses[name]?.length == 1) {
                ...
                Debug(
                    "test",
                    `yoo issue #4 (${variable} = ${name} and ${name} has 1 use.), probably ${reassigned ? "returning" : `inlining ${name}`}`,
                );
            ...
            if (
                ...
                // uh so && !reassigned is because if it's a re-assign we know we're FORCED to inline it :(
            ...
            if (length == 0) {
                ...
                //}
                    ...
                    return true;
                } else if ((reassigned || isFinally) && (isSimple(init) || init.type === "IndexExpression" || init.type === "MemberExpression")) {
                ...
                return Debug(
                    ...
                    "(has 0 uses)",
                    init.type,
                    reassigned ? "(reassigned)" : "(wasnt reassigned)",
                );
            }

            //if (!canInline && isDifferentScope(variable))// && !reassigned)
            ...
            if (!canInline) {
                if (isDifferentScope(variable) || (list.length == 1 && globalDifferentScopes.has(list[0])))
                    return Debug("Can't inline", variable, init.type, length);
                //Debug("test", "bro what",variable,list[0])
            }
        }
    ...
    const finallyInline = (uses, values) => {
        for (let variable in uses) {
            ...
            inlineThing(value, list, variable, false, false, false, true); // Pass isFinally=true
        }
    };
